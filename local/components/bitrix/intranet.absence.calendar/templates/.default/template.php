...
<div id="bx_calendar_control_datepicker" style="display: none">
<input type="hidden" id="bx_goto_date" name="bx_goto_date" value="<?echo ConvertTimeStamp();?>" />
<img src="/bitrix/js/main/core/images/calendar-icon.gif" class="calendar-icon" onclick="BX.calendar({node:this, field:'bx_goto_date', bTime: false, callback: jsBXAC.InsertDate});" onmouseover="BX.addClass(this, 'calendar-icon-hover');" onmouseout="BX.removeClass(this, 'calendar-icon-hover');" border="0"/>
</div>

<?
$obUser = new CUser();
$dbUsers = $obUser->GetList(($sort_by = 'LAST_NAME'), ($sort_dir = 'asc'), array("ACTIVE"=>"Y"),array("FIELDS"=>array("ID","NAME","LAST_NAME")));
$arUsers = array();
...
?>
<div id="bx_calendar_control_users" style="display:none;">
	<select id="user_select" class='userSelectFilter'>
		<option value="all"  <?=($_REQUEST['user']=='all' || !isset($_REQUEST['user']))?'selected':''?>>Все</option>
		<?foreach($arUsers as $user):?>
			<option value="<?=$user["ID"]?>" <?=($_REQUEST['user']==$user['ID'])?'selected':''?>><?=$user["LAST_NAME"].' '.$user['NAME']?></option>
		<?endforeach;?>
	</select>
</div>
