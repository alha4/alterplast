...
/* methods  */
	Init: function (arParams)
	{
		...
		this.SETTINGS.USER = arParams.USER;
		this.SETTINGS.USERS_ALL = 'N';
	...
	LoadData: function(ts_start, ts_finish)
	{
		...
		var url = this.LOADER
			...
			+ '&user=' +  parseInt(this.SETTINGS.USER)
	...
	ShowLayout: function()
	{
		...
		var obUsersContainer = document.getElementById('user_select');
		if (null != obUsersContainer)
		{
			this.CONTROLS.USERS = obUsersContainer;
			this.CONTROLS.USERS.onchange = function() {

				_this.SETTINGS.USER = this.value;

				_this.SetDataFilter('user', this.value);

			/*	var search=document.location.search;
				if(document.location.search!='')
				{
					var start,delimetr,value;
					if(start=search.indexOf('user=',0)+1)
					{
						delimetr=search.indexOf('&',start)
						if(delimetr+1!=0)
						{
							value=search.substr(start-1,delimetr-start+1);
						}
						else
						{
							value=search.substr(start-1);
						}
						search=search.replace(value,"user="+this.value);
						document.location.search=search;
					}
					else
						document.location.search=document.location.search+'&user='+this.value;
				}
				else
					document.location.search='?user='+this.value;
			};*/

		 }

			this.TOOLBAR.BXAddControl("Пользователь", this.CONTROLS.USERS);
		}
	},
