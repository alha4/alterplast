...
<?
$res=CIBlockElement::GetList(array(),array("IBLOCK_ID"=>IB_EVENTS_TYPE),false,false,array("ID","NAME","PROPERTY_EVENT_COLOR","PROPERTY_EVENT_TEXT_COLOR","PROPERTY_NEED_CONFIRM"));
while($arEventType=$res->Fetch())
{
	$TYPES[] = "{NAME:'".$arEventType["ID"]."',TITLE:'".$arEventType["NAME"]."'}";
	//$TYPE_BGCOLORS[] = "{'".$arEventType["ID"]."':{BG:'".$arEventType['PROPERTY_EVENT_COLOR_VALUE']."',COLOR:'".$arEventType['PROPERTY_EVENT_TEXT_COLOR_VALUE']."'}}";
	?>
		<script>
			var a = new Object();
			a.BG='<?=$arEventType['PROPERTY_EVENT_COLOR_VALUE']?>';
			a.COLOR='<?=$arEventType['PROPERTY_EVENT_TEXT_COLOR_VALUE']?>';
			a.CONFIRM='<?=$arEventType['PROPERTY_NEED_CONFIRM_VALUE']?>';
			types[<?=$arEventType["ID"]?>] = a;
		</script>
	<?
	$arTypes[]=$arEventType;
}
...
jsBXAC.Init(
	{
		'LOADER': '/bitrix/components/bitrix/intranet.absence.calendar/ajax.php',
               /* 'LOADER': '/bitrix/components/areal/intranet.absence.calendar/ajax.php',*/
