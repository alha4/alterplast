...
function JCCalendarFilter(_callback, _parentNode, _arTypes, MESSAGES, typeBgColors)
{
	...
	for (var i=0; i < this.arTypes.length; i++)
	{
		...
		obDiv.style.background = typeBgColors[this.arTypes[i].NAME].BG;
		obDiv.style.color = typeBgColors[this.arTypes[i].NAME].COLOR;
			...
			_this.DIV.innerHTML = '<div class="bx-calendar-info-header"><a class="bx-calendar-info-close" id="bx_top_close_' + _this.TYPE + '_' + _this.ID + '"></a></div>'
				+ '<div class="bx-calendar-info-data">'
					...
					+ '<div class="bx-calendar-info-data-cont">'
						...
						+ '</div>'
						+ '<div class="bx-info-entry bx-calendar-color-' + _this.INFO.ENTRY.TYPE + '" style="background: '+_this.INFO.ENTRY.ABSENCE_TYPE.PROPERTY_EVENT_COLOR_VALUE+';color:'+_this.INFO.ENTRY.ABSENCE_TYPE.PROPERTY_EVENT_TEXT_COLOR_VALUE+'">'
							+ _this.INFO.ENTRY.ABSENCE_TYPE.NAME
						+ '</div>'
